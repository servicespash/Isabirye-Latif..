...
var RE2Flags = class _RE2Flags {
  ...
  static MATCH_NL = (() => _RE2Flags.CLASS_NL | _RE2Flags.DOT_NL)();
  // As close to Perl as possible.
  static PERL = (() => _RE2Flags.CLASS_NL | _RE2Flags.ONE_LINE | _RE2Flags.PERL_X | _RE2Flags.UNICODE_GROUPS)();
  // POSIX syntax.
  static POSIX = 0;
  //// Anchors
  static UNANCHORED = 0;
...
var UnicodeTables = class _UnicodeTables {
  ...
  static Meroitic_Cursive = [[68e3, 68023, 1], [68028, 68047, 1], [68050, 68095, 1]];
  ...
  static Tagbanwa = [[5984, 5996, 1], [5998, 6e3, 1], [6002, 6003, 1]];
...
var MatcherInputBase = class _MatcherInputBase {
   ...
   */
  isUTF8Encoding() {
    return this.getEncoding() === _MatcherInputBase.Encoding.UTF_8;
  }
  /**
   *
   * @returns {boolean}
   */
  isUTF16Encoding() {
    return this.getEncoding() === _MatcherInputBase.Encoding.UTF_16;
  }
};
...
var Regexp = class _Regexp {
  ...
  static isPseudoOp(op) {
    return op >= _Regexp.Op.LEFT_PAREN;
  }
  static emptySubs() {
    return [];
  }
  ...
  appendTo() {
    let out = "";
    switch (this.op) {
      case _Regexp.Op.NO_MATCH:
        out += "[^\\x00-\\x{10FFFF}]";
        break;
      case _Regexp.Op.EMPTY_MATCH:
        out += "(?:)";
        break;
      case _Regexp.Op.STAR:
      case _Regexp.Op.PLUS:
      case _Regexp.Op.QUEST:
      case _Regexp.Op.REPEAT: {
        const sub = this.subs[0];
        if (sub.op > _Regexp.Op.CAPTURE || sub.op === _Regexp.Op.LITERAL && sub.runes.length > 1) {
          out += `(?:${sub.appendTo()})`;
        } else {
          out += sub.appendTo();
        }
      ...
      case _Regexp.Op.CONCAT: {
        for (let sub of this.subs) {
          if (sub.op === _Regexp.Op.ALTERNATE) {
            out += `(?:${sub.appendTo()})`;
          } else {
            out += sub.appendTo();
          }
      ...
      case _Regexp.Op.LITERAL:
        ...
        break;
      case _Regexp.Op.ANY_CHAR_NOT_NL:
        out += "(?-s:.)";
        break;
      case _Regexp.Op.ANY_CHAR:
        out += "(?s:.)";
        break;
      case _Regexp.Op.CAPTURE:
        if (this.name === null || this.name.length === 0) {
          out += "(";
        } else {
          out += `(?P<${this.name}>`;
        }
        if (this.subs[0].op !== _Regexp.Op.EMPTY_MATCH) {
          out += this.subs[0].appendTo();
        }
        out += ")";
        break;
      case _Regexp.Op.BEGIN_TEXT:
        out += "\\A";
        break;
      case _Regexp.Op.END_TEXT:
        ...
        break;
      case _Regexp.Op.BEGIN_LINE:
        out += "^";
        break;
      case _Regexp.Op.END_LINE:
        out += "$";
        break;
      case _Regexp.Op.WORD_BOUNDARY:
        out += "\\b";
        break;
      case _Regexp.Op.NO_WORD_BOUNDARY:
        out += "\\B";
        break;
      case _Regexp.Op.CHAR_CLASS:
        ...
        if (this.runes.length === 0) {
          ...
          for (let i = 1; i < this.runes.length - 1; i += 2) {
            ...
            out += _Regexp.quoteIfHyphen(lo);
            out += Utils.escapeRune(lo);
            if (lo !== hi) {
              out += "-";
              out += _Regexp.quoteIfHyphen(hi);
  ...
  equals(that) {
    if (!(that !== null && that instanceof _Regexp)) {
      return false;
    }
    if (this.op !== that.op) {
      return false;
...
var Inst = class _Inst {
  ...
  static isRuneOp(op) {
    return _Inst.RUNE <= op && op <= _Inst.RUNE_ANY_NOT_NL;
  }
  ...
  toString() {
    switch (this.op) {
      ...
      case _Inst.RUNE:
        ...
        return ["rune ", _Inst.escapeRunes(this.runes), (this.arg & RE2Flags.FOLD_CASE) !== 0 ? "/i" : "", " -> ", this.out].join("");
      case _Inst.RUNE1:
        return `rune1 ${_Inst.escapeRunes(this.runes)} -> ${this.out}`;
      case _Inst.RUNE_ANY:
        return `any -> ${this.out}`;
      case _Inst.RUNE_ANY_NOT_NL:
  ...
  compile(re) {
    switch (re.op) {
      ...
      case Regexp.Op.ANY_CHAR:
        return this.rune(_Compiler.ANY_RUNE(), 0);
...
var CharClass = class _CharClass {
  ...
  cleanClass() {
    ...
    _CharClass.qsortIntPair(this.r, 0, this.len - 2);
    let w = 2;
  ...
  appendLiteral(x2, flags) {
    return (flags & RE2Flags.FOLD_CASE) !== 0 ? this.appendFoldedRange(x2, x2) : this.appendRange(x2, x2);
  }
  ...
  appendClassWithSign(x2, sign) {
    return sign < 0 ? this.appendNegatedClass(x2) : this.appendClass(x2);
  }
  ...
  appendGroup(g, foldCase) {
    let cls = g.cls;
    if (foldCase) {
      cls = new _CharClass().appendFoldedClass(cls).cleanClass().toArray();
    }
    return this.appendClassWithSign(cls, g.sign);
  }
  toString() {
    return _CharClass.charClassToString(this.r, this.len);
  }
};
...
var Parser = class _Parser {
  ...
  static unicodeTable(name3) {
    ...
    if (UnicodeTables.CATEGORIES.has(name3)) {
      return Pair.of(UnicodeTables.CATEGORIES.get(name3), UnicodeTables.FOLD_CATEGORIES.get(name3));
    }
    if (UnicodeTables.SCRIPTS.has(name3)) {
      return Pair.of(UnicodeTables.SCRIPTS.get(name3), UnicodeTables.FOLD_SCRIPT.get(name3));
    }
    return null;
  }
  ...
  static parse(pattern, flags) {
    return new _Parser(pattern, flags).parseInternal();
  }
  ...
  static parseRepeat(t) {
    ...
    if (!t.lookingAt(",")) {
      ...
      if (t.lookingAt("}")) {
        max = -1;
      } else if ((max = _Parser.parseInt(t)) === -1) {
        return -1;
      }
    }
    if (!t.more() || !t.lookingAt("}")) {
    ...
    if (min < 0 || min > 1e3 || max === -2 || max > 1e3 || max >= 0 && min > max) {
      throw new RE2JSSyntaxException(_Parser.ERR_INVALID_REPEAT_SIZE, t.from(start));
    }
  ...
  static mergeCharClass(dst, src) {
    switch (dst.op) {
      case Regexp.Op.ANY_CHAR:
        break;
      case Regexp.Op.ANY_CHAR_NOT_NL:
        if (_Parser.matchRune(src, Codepoint.CODES.get("\n"))) {
  ...
  static parseEscape(t) {
    ...
    t.skip(1);
    if (!t.more()) {
      throw new RE2JSSyntaxException(_Parser.ERR_TRAILING_BACKSLASH);
    }
    let c = t.pop();
    bigswitch: switch (c) {
    ...
    throw new RE2JSSyntaxException(_Parser.ERR_INVALID_ESCAPE, t.from(startPos));
  }
  ...
  static parseClassChar(t, wholeClassPos) {
    if (!t.more()) {
      throw new RE2JSSyntaxException(_Parser.ERR_MISSING_BRACKET, t.from(wholeClassPos));
    }
    if (t.lookingAt("\\")) {
      return _Parser.parseEscape(t);
    }
    return t.pop();
  }
  static concatRunes(x2, y) {
    return [...x2, ...y];
  }
  ...
  maybeConcat(r, flags) {
    ...
    re2.runes = _Parser.concatRunes(re2.runes, re1.runes);
    if (r >= 0) {
      re1.runes = [r];
      re1.flags = flags;
      return true;
  ...
  repeat(op, min, max, beforePos, t, lastRepeatPos) {
    ...
    if ((flags & RE2Flags.PERL_X) !== 0) {
      ...
      if (lastRepeatPos !== -1) {
        throw new RE2JSSyntaxException(_Parser.ERR_INVALID_REPEAT_OP, t.from(lastRepeatPos));
      }
    }
    const n = this.stack.length;
    if (n === 0) {
  ...
  factor(array2) {
    ...
    for (let i = 0; i <= lensub; i++) {
      let ifirst = null;
      if (i < lensub) {
        ifirst = _Parser.leadingRegexp(array2[s + i]);
    ...
    lenout = 0;
    for (let i = 0; i <= lensub; i++) {
      if (i < lensub && _Parser.isCharClass(array2[s + i])) {
        continue;
      }
      if (i === start) ;
      else if (i === start + 1) {
    ...
    lenout = 0;
    for (let i = 0; i < lensub; ++i) {
      if (i + 1 < lensub && array2[s + i].op === Regexp.Op.EMPTY_MATCH && array2[s + i + 1].op === Regexp.Op.EMPTY_MATCH) {
        continue;
      }
      array2[lenout++] = array2[s + i];
    }
  ...
  parseInternal() {
    if ((this.flags & RE2Flags.LITERAL) !== 0) {
      return _Parser.literalRegexp(this.wholeRegexp, this.flags);
    }
    let lastRepeatPos = -1;
    let min = -1;
    let max = -1;
    ...
    while (t.more()) {
      {
        ...
        bigswitch: switch (t.peek()) {
          ...
          case Codepoint.CODES.get("{"): {
            repeatPos = t.pos();
            const minMax = _Parser.parseRepeat(t);
            if (minMax < 0) {
            ...
            if ((this.flags & RE2Flags.PERL_X) !== 0 && t.more()) {
              ...
              switch (c) {
                ...
                case Codepoint.CODES.get("C"):
                  throw new RE2JSSyntaxException(_Parser.ERR_INVALID_ESCAPE, "\\C");
    ...
    const n = this.stack.length;
    if (n !== 1) {
      throw new RE2JSSyntaxException(_Parser.ERR_MISSING_PAREN, this.wholeRegexp);
    }
  ...
  parsePerlFlags(t) {
    ...
    if (s.startsWith("(?P<") || s.startsWith("(?<")) {
      ...
      t.skipString(name3);
      t.skip(begin + 1);
      if (!_Parser.isValidCaptureName(name3)) {
      ...
      re.cap = ++this.numCap;
      if (this.namedGroups[name3]) {
        throw new RE2JSSyntaxException(_Parser.ERR_DUPLICATE_NAMED_CAPTURE, name3);
      }
    ...
    throw new RE2JSSyntaxException(_Parser.ERR_INVALID_PERL_OP, t.from(startPos));
  }
  ...
  swapVerticalBar() {
    const n = this.stack.length;
    if (n >= 3 && this.stack[n - 2].op === Regexp.Op.VERTICAL_BAR && _Parser.isCharClass(this.stack[n - 1]) && _Parser.isCharClass(this.stack[n - 3])) {
      ...
      _Parser.mergeCharClass(re3, re1);
      this.reuse(re1);
      this.pop();
      return true;
    }
  ...
  parseRightParen() {
    ...
    const n = this.stack.length;
    if (n < 2) {
      throw new RE2JSSyntaxException(_Parser.ERR_INTERNAL_ERROR, "stack underflow");
    }
    const re1 = this.pop();
    const re2 = this.pop();
  ...
  parseNamedClass(t, cc) {
    ...
    if (g === null) {
      throw new RE2JSSyntaxException(_Parser.ERR_INVALID_CHAR_RANGE, name3);
    }
    cc.appendGroup(g, (this.flags & RE2Flags.FOLD_CASE) !== 0);
    return true;
  }
  ...
  parseUnicodeClass(t, cc) {
    ...
    const pair = _Parser.unicodeTable(name3);
    if (pair === null) {
  ...
  parseClass(t) {
    ...
    while (!t.more() || t.peek() !== Codepoint.CODES.get("]") || first) {
      ...
      t.rewindTo(beforePos);
      const lo = _Parser.parseClassChar(t, startPos);
      let hi = lo;
      if (t.more() && t.lookingAt("-")) {
        t.skip(1);
        if (t.more() && t.lookingAt("]")) {
          ...
          hi = _Parser.parseClassChar(t, startPos);
          if (hi < lo) {
            throw new RE2JSSyntaxException(_Parser.ERR_INVALID_CHAR_RANGE, t.from(beforePos));
          }
        }
      }
      if ((this.flags & RE2Flags.FOLD_CASE) === 0) {
...
var RE2 = class _RE2 {
  ...
  static initTest(expr) {
    const re2 = _RE2.compile(expr);
    const res = new _RE2(re2.expr, re2.prog, re2.numSubexp, re2.longest);
    res.cond = re2.cond;
   ...
   */
  static compile(expr) {
    return _RE2.compileImpl(expr, RE2Flags.PERL, false);
  }
  /**
  ...
  static compilePOSIX(expr) {
    return _RE2.compileImpl(expr, RE2Flags.POSIX, true);
  }
  ...
  static match(pattern, s) {
    return _RE2.compile(pattern).match(s);
  }
  ...
  allMatches(input, n, deliverFun = (v2) => v2) {
    let result = [];
  ...
  // This is visible for testing.
  findAllUTF8(b, n) {
    const result = this.allMatches(MachineInput.fromUTF8(b), n, (match2) => b.slice(match2[0], match2[1]));
    if (result.length === 0) {
      return null;
    }
    return result;
  ...
  findAllUTF8Index(b, n) {
    const result = this.allMatches(MachineInput.fromUTF8(b), n, (match2) => match2.slice(0, 2));
    if (result.length === 0) {
      return null;
    }
    return result;
  ...
  // This is visible for testing.
  findAll(s, n) {
    const result = this.allMatches(MachineInput.fromUTF16(s), n, (match2) => s.substring(match2[0], match2[1]));
    if (result.length === 0) {
      return null;
    }
    return result;
  ...
  // This is visible for testing.
  findAllIndex(s, n) {
    const result = this.allMatches(MachineInput.fromUTF16(s), n, (match2) => match2.slice(0, 2));
    if (result.length === 0) {
      return null;
    }
    return result;
  ...
  findAllUTF8Submatch(b, n) {
    const result = this.allMatches(MachineInput.fromUTF8(b), n, (match2) => {
      let slice = new Array(match2.length / 2 | 0).fill(null);
      for (let j = 0; j < slice.length; j++) {
        if (match2[2 * j] >= 0) {
          slice[j] = b.slice(match2[2 * j], match2[2 * j + 1]);
        }
      }
      return slice;
    });
  ...
  // This is visible for testing.
  findAllSubmatch(s, n) {
    const result = this.allMatches(MachineInput.fromUTF16(s), n, (match2) => {
      let slice = new Array(match2.length / 2 | 0).fill(null);
      for (let j = 0; j < slice.length; j++) {
        if (match2[2 * j] >= 0) {
          slice[j] = s.substring(match2[2 * j], match2[2 * j + 1]);
        }
      }
      return slice;
    });
...
var RE2JS = class _RE2JS {
  ...
  static compile(regex, flags = 0) {
    let fregex = regex;
    if ((flags & _RE2JS.CASE_INSENSITIVE) !== 0) {
      fregex = `(?i)${fregex}`;
    }
    if ((flags & _RE2JS.DOTALL) !== 0) {
      fregex = `(?s)${fregex}`;
    }
    if ((flags & _RE2JS.MULTILINE) !== 0) {
      fregex = `(?m)${fregex}`;
    }
    if ((flags & ~(_RE2JS.MULTILINE | _RE2JS.DOTALL | _RE2JS.CASE_INSENSITIVE | _RE2JS.DISABLE_UNICODE_GROUPS | _RE2JS.LONGEST_MATCH)) !== 0) {
    ...
    if ((flags & _RE2JS.DISABLE_UNICODE_GROUPS) !== 0) {
      re2Flags &= ~RE2Flags.UNICODE_GROUPS;
    }
    const p = new _RE2JS(regex, flags);
    p.re2Input = RE2.compileImpl(fregex, re2Flags, (flags & _RE2JS.LONGEST_MATCH) !== 0);
    return p;
  }
  /**
  ...
  static matches(regex, input) {
    return _RE2JS.compile(regex).matcher(input).matches();
  }
  /**
   * This is visible for testing.
   * @private
  ...
  static initTest(pattern, flags, re2) {
    ...
    const p = new _RE2JS(pattern, flags);
    p.re2Input = re2;
    return p;
  }
  /**
...
var IndexOffset = class _IndexOffset {
  ...
  static min() {
    return new _IndexOffset(SnapshotVersion.min(), DocumentKey.empty(), L);
  }
  /** Returns an offset that sorts after all regular offsets. */
  static max() {
    return new _IndexOffset(SnapshotVersion.max(), DocumentKey.empty(), L);
  }
};
...
var We = "globals";
var Ge = [...[...[...[...[z, H, ee, W, ue, Q, Ie, Ee], de], oe], Ae], me, ge];
var ze = [...Ge, Be];
var je = [z, H, ee, te, ue, Q, Ie, Ee, de, oe, Ae, me, ge, Be];
var He = je;
var Je = [...He, we, De, Oe];
var Ze = [...Je, We];
...
var ft = new SortedSet(__PRIVATE_primitiveComparator);
function __PRIVATE_targetIdSet() {
  return ft;
}
var pt = new Integer([4294967295, 4294967295], 0);
...
var __PRIVATE_WebChannelConnection = class ___PRIVATE_WebChannelConnection extends __PRIVATE_RestConnection {
  ...
  cn(e, t, n) {
    ...
    ___PRIVATE_WebChannelConnection.rn(), setTimeout((() => {
      E.Xt();
    }), 0), E;
  }
  /**
...
var kt = "firestore.googleapis.com";
var qt = true;
...
function __PRIVATE_fieldPathFromDotSeparatedString(e, t, n) {
  if (t.search(Kt) >= 0) throw createError(
...
var AggregateFunction = class _AggregateFunction {
  ...
  static _create(e, t, n) {
    const r = new _AggregateFunction(e, t);
...
var Constant = class _Constant extends Expression {
   ...
   */
  static _fromProto(e) {
    const t = new _Constant(e, void 0);
...
var BooleanExpression = class _BooleanExpression extends Expression {
  ...
  ifError(e) {
    ...
    return t instanceof _BooleanExpression ? n.asBoolean() : n;
  }
  /**
   * @private
   * @internal
...
var __PRIVATE_Replace = class ___PRIVATE_Replace extends Stage {
  ...
  _toProto(e) {
    return {
      ...super._toProto(e),
      args: [this.map._toProto(e), __PRIVATE_toStringValue(___PRIVATE_Replace.pr)]
    };
  }
  _readUserData(e) {
...
var __PRIVATE_EvaluateResult = class ___PRIVATE_EvaluateResult {
  constructor(e, t) {
    this.type = e, this.value = t;
  }
  static vr() {
    return new ___PRIVATE_EvaluateResult("ERROR", void 0);
  }
  static Sr() {
    return new ___PRIVATE_EvaluateResult("UNSET", void 0);
  }
  static Dr() {
    return new ___PRIVATE_EvaluateResult("NULL", lt);
  }
  static newValue(e) {
    return __PRIVATE_isNullValue(e) ? new ___PRIVATE_EvaluateResult("NULL", lt) : (function __PRIVATE_isBoolean(e2) {
      return !!e2 && "booleanValue" in e2;
    })(e) ? new ___PRIVATE_EvaluateResult("BOOLEAN", e) : isInteger(e) ? new ___PRIVATE_EvaluateResult("INT", e) : __PRIVATE_isDouble(e) ? new ___PRIVATE_EvaluateResult("DOUBLE", e) : (function __PRIVATE_isTimestampValue(e2) {
      return !!e2 && "timestampValue" in e2 && !!e2.timestampValue;
    })(e) ? new ___PRIVATE_EvaluateResult("TIMESTAMP", e) : (function __PRIVATE_isString(e2) {
      return !!e2 && "stringValue" in e2;
    })(e) ? new ___PRIVATE_EvaluateResult("STRING", e) : (function __PRIVATE_isBytes(e2) {
      return !!e2 && "bytesValue" in e2;
    })(e) ? new ___PRIVATE_EvaluateResult("BYTES", e) : e.referenceValue ? new ___PRIVATE_EvaluateResult("REFERENCE", e) : e.geoPointValue ? new ___PRIVATE_EvaluateResult("GEO_POINT", e) : isArray(e) ? new ___PRIVATE_EvaluateResult("ARRAY", e) : __PRIVATE_isVectorValue(e) ? new ___PRIVATE_EvaluateResult("VECTOR", e) : __PRIVATE_isMapValue(e) ? new ___PRIVATE_EvaluateResult("MAP", e) : new ___PRIVATE_EvaluateResult("ERROR", void 0);
  }
  Cr() {
...
var Wt = BigInt("0x7fffffffffffffff");
var Qt = -BigInt("0x8000000000000000");
...
var __PRIVATE_CoreXor = class ___PRIVATE_CoreXor {
  constructor(e) {
    this.expr = e;
  }
  evaluate(e, t) {
    ...
    for (const i of this.expr.params) {
      ...
      switch (s.type) {
        case "BOOLEAN":
          n = ___PRIVATE_CoreXor.xor(n, !!s.value?.booleanValue);
          break;
        case "NULL":
          r = true;
          break;
...
var Gt = BigInt(-62135596800);
var zt = BigInt(253402300799);
var jt = BigInt(1e3);
var Ht = BigInt(1e6);
var Jt = Gt * jt;
var Yt = zt * jt + BigInt(999);
var Zt = Gt * Ht;
var Xt = zt * Ht + BigInt(999999);
...
var __PRIVATE_FirestoreIndexValueWriter = class {
  ...
  Yr(e, t) {
    this.Zr(e, t), // Write separator to split index values
    // (see go/firestore-storage-format#encodings).
    t.Xr();
  }
  Zr(e, t) {
    if ("nullValue" in e) this.ei(t, 5);
    else if ("booleanValue" in e) this.ei(t, 10), t.ti(e.booleanValue ? 1 : 0);
    else if ("integerValue" in e) this.ei(t, 15), t.ti(__PRIVATE_normalizeNumber(e.integerValue));
      ...
      this.ei(t, 20), "string" == typeof n && (n = __PRIVATE_normalizeTimestamp(n)), t.ni(`${n.seconds || ""}`), t.ti(n.nanos || 0);
    } else if ("stringValue" in e) this.ri(e.stringValue, t), this.ii(t);
    else if ("bytesValue" in e) this.ei(t, 30), t.si(__PRIVATE_normalizeByteString(e.bytesValue)), this.ii(t);
    else if ("referenceValue" in e) this._i(e.referenceValue, t);
    else if ("geoPointValue" in e) {
      const n = e.geoPointValue;
      this.ei(t, 45), t.ti(n.latitude || 0), t.ti(n.longitude || 0);
    } else "mapValue" in e ? __PRIVATE_isMaxValue(e) ? this.ei(t, Number.MAX_SAFE_INTEGER) : __PRIVATE_isVectorValue(e) ? this.oi(e.mapValue, t) : (this.ai(e.mapValue, t), this.ii(t)) : "arrayValue" in e ? (this.ui(e.arrayValue, t), this.ii(t)) : fail(19022, {
      ci: e
    });
  }
  ri(e, t) {
  ...
  ai(e, t) {
    const n = e.fields || {};
    this.ei(t, 55);
    for (const e2 of Object.keys(n)) this.ri(e2, t), this.Zr(n[e2], t);
  }
  oi(e, t) {
    const n = e.fields || {};
    this.ei(t, 53);
    const r = ct, i = n[r].arrayValue?.values?.length || 0;
    this.ei(t, 15), t.ti(__PRIVATE_normalizeNumber(i)), // Vectors then sort by position value
    this.ri(r, t), this.Zr(n[r], t);
  }
  ui(e, t) {
    const n = e.values || [];
    this.ei(t, 50);
    for (const e2 of n) this.Zr(e2, t);
  }
  _i(e, t) {
    this.ei(t, 37);
    DocumentKey.fromName(e).path.forEach(((e2) => {
      this.ei(t, 60), this.li(e2, t);
    }));
  }
  ei(e, t) {
    e.ti(t);
...
var DelayedOperation = class _DelayedOperation {
  ...
  static createAndSchedule(e, t, n, r, i) {
    const s = Date.now() + n, _ = new _DelayedOperation(e, t, s, r, i);
    return _.start(n), _;
  }
  /**
...
!(function __PRIVATE_registerFirestore(h, d = true) {
  ...
  }), "PUBLIC").setMultipleInstances(true)), registerVersion(Yt2, Kt2, h), // BUILD_TARGET will be replaced by values like esm, cjs, etc during the compilation
  registerVersion(Yt2, Kt2, "esm2020");
})();
...
var databaseId = "ai-studio-lattyportfolio-88c45d5c-6597-4b50-9afe-23b9a63e9921";
var db = null;
function getDb() {
  if (!db) {
    try {
      const app2 = initializeApp(firebaseConfig);
      db = getFirestore(app2, databaseId);
    } catch (e) {
...
app.get("/api/v1/download", async (c) => {
  ...
  if (!appType || !version3 || appType !== "study" && appType !== "resonance") {
  ...
  return c.redirect(`https://github.com/IsabiryeLatif/cymatic-evolution/releases/download/${version3}/${appType}-${version3}.zip`);
});
